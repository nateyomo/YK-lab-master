...
<p>Item 5 Current: <span id="currentScore5">0</span></p>
...
<p>Item 6 Current: <span id="currentScore6">0</span></p>
...
<h2>Click to Assign Score to Item 1</h2>
<table class="basic-table">
    <thead>
        <tr>
            <th>Score</th>
...
<p>Current Score1: <span id="currentScore1">0</span></p>
...
<p>Current Score2: <span id="currentScore2">0</span></p>
...
<table class="basic-table">
    ...
    <tbody>
        ...
        <tr>
            <td>Final Time</td>
            <td id="finalLLETime" colspan="2">--- s</td>
            <td id="finalRLETime" colspan="2">--- s</td>
        </tr>
        <tr>
            <td>Final Score</td>
            <td id="finalLLEScore" colspan="2">---</td>
            <td id="finalRLEScore" colspan="2">---</td>
        </tr>
    </tbody>
</table>
...
<script>
    ...
    function updateTimer(index, resultId, buttonId) {
        ...
        if (elapsedTimes[index] < 20000) {
            ...
            timers[index] = requestAnimationFrame(() => updateTimer(index, resultId, buttonId));
        } else {
    ...
    function stopTimer(index, resultId, buttonId = null) {
        if (isRunning[index]) {
            ...
            if (buttonId) {
                document.getElementById(buttonId).innerText = "Start Timer"; // Reset button text
            }
        }
        checkLongestTimes();
    }
    ...
    function checkLongestTimes() {
        ...
        const lleValid = lleTimes.filter(time => time !== -Infinity);
        ...
        const rleValid = rleTimes.filter(time => time !== -Infinity);

        // Set LLE Final Time
        let finalLLETime;
        if (lleValid.length > 0) {
            finalLLETime = Math.max(...lleValid);
            document.getElementById("finalLLETime").innerText = `${(finalLLETime / 1000).toFixed(3)} s`;
        } else {
            finalLLETime = "Unable";
            document.getElementById("finalLLETime").innerText = "Unable";
        }

        // Set RLE Final Time
        let finalRLETime;
        if (rleValid.length > 0) {
            finalRLETime = Math.max(...rleValid);
            document.getElementById("finalRLETime").innerText = `${(finalRLETime / 1000).toFixed(3)} s`;
        } else {
            finalRLETime = "Unable";
            document.getElementById("finalRLETime").innerText = "Unable";
        }

        // Update scores based on final times
        const finalLLEScore = item3calculateScore(finalLLETime);
        const finalRLEScore = item3calculateScore(finalRLETime);

        document.getElementById("finalLLEScore").innerText = finalLLEScore;
        document.getElementById("finalRLEScore").innerText = finalRLEScore;

        // Update the smallest time bolding
        boldSmallestTime();

        // Bold the scores if their corresponding final times are bold
        syncBoldWithTimes("finalLLETime", "finalLLEScore");
        syncBoldWithTimes("finalRLETime", "finalRLEScore");
    ...
    function item3calculateScore(finalTime) {
        ...
        return "Undefined"; // Optional, could be omitted if not needed
    }
    ...
    function boldSmallestTime() {
        const times = [
            { id: "finalLLETime", value: document.getElementById("finalLLETime").innerText },
            { id: "finalRLETime", value: document.getElementById("finalRLETime").innerText },
        ];
    ...
    function setScore3() {
        const lleTime = document.getElementById("finalLLETime").innerText;
        const rleTime = document.getElementById("finalRLETime").innerText;

        // Reset currentScore3 to 0 by default
        let currentScore3 = 0; 

        // Check which time is bolded and calculate the corresponding score
        if (document.getElementById("finalLLETime").style.fontWeight === "bold") {
            currentScore3 = item3calculateScore(lleTime);
        } else if (document.getElementById("finalRLETime").style.fontWeight === "bold") {
            currentScore3 = item3calculateScore(rleTime);
        }
...
<p>Current Score3: <span id="currentScore3">0</span></p>
